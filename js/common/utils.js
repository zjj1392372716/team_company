 ...
 * 函数名: toTreeData
 * 描  述:
 * <summary>将数组数据转换成树节点数据</summary>
 * 参  数:
 * <param name="aSrc">     - 源数据</param>
 ...
 * <remarks></remarks>
 * =======================================================================================*/
utils.toTreeData = function (aSrc, ID, ParentID)
{
    var aNode = [];                         // 节点
    var aHash = {};                         // 以索引为下标的数组
    var nLength = (aSrc || []).length;      // 长度

    // 转换成以索引为下标的数组
    for (var i = 0; i < nLength; i++)
    {
        aHash[aSrc[i][ID]] = aSrc[i];
    }

    for (var j = 0; j < nLength; j++)
    {
        var aVal = aSrc[j]
        var ParentNode = aHash[aVal[ParentID]];
        if (ParentNode)
        {
            !ParentNode['children'] && (ParentNode['children'] = []);
            ParentNode['children'].push(aVal);
        }
        else
        {
            aNode.push(aVal);
...
utils.eachTreeRow = function (treeData, eachHandler)
{
    ...
    {
        if (eachHandler(treeData[i]) == false)
        {
            break;
        }
 ...
 * 参  数:
 * <param name="obj">                - 要进行属性操作的对象</param>
 * <param name="sourcePropertyName"> - 源属性名称</param>
 * <param name="newPropertyName">    - 新属性名称</param>
 * <param name="overWrite">          - 该属性已存在，是否覆盖标志</param>
 * 返回值:
 * <returns></returns>
 * 说  明:
 * <remarks>这是一个递归函数</remarks>
 * =======================================================================================*/
