...
com.viewModel.searchEdit = function (data) {
    ...
    this.gridEdit = new comm.editGridViewModel(this.grid);
    ...
    this.grid.OnAfterCreateEditor = function (editors) {
        if (editors[self.setting.idField]) comm.readOnlyHandler('input')(editors[self.setting.idField].target, true);
    };
    ...
    this.editClick = function () {
        ...
        if (!row) return comm.message('warning', self.resx.noneSelect);
        self.gridEdit.begin()
    };
    ...
    this.auditClick = function () {
        var row = self.grid.datagrid('getSelected');
        if (!row) return comm.message('warning', self.resx.noneSelect);
        comm.auditDialog(function (d) {
            comm.ajax({
                type: 'POST',
                url: self.urls.audit + row.BillNo,
                data:JSON.stringify(d),
                success: function () {
                    comm.message('success', self.resx.auditSuccess);
                }
            });
        });
    };
    ...
    this.downloadClick = function (vm, event) {
        comm.exporter(self.grid).download($(event.currentTarget).attr("suffix"));
    };
    this.copyClick = function () {
        var source = self.grid.datagrid('getSelected');
        if (!source) return comm.message('warning', self.resx.noneSelect);
        comm.ajax({
            type: 'GET',
            url: self.urls.newkey,
            success: function (d) {
