...
com.viewModel.edit = function (data) {
    ...
    this.rejectClick = function () {
        ...
        comm.message('success', self.resx.rejected);
    };
    ...
    this.saveClick = function () {
        ...
        if (validMessage) {
            comm.message('warning', validMessage);
            return;
        }
        ...
        if (!post._changed) {
            comm.message('success', '页面没有数据被修改！');
            return;
        }
        ...
        comm.ajax({
            ...
            data: ko.toJSON(post),
            success: function (d) {
                comm.message('success', self.resx.editSuccess);
    ...
    this.fnIsPageChanged = function () {
        ...
        result.form = comm.formChanges(self.pageData.form, data.dataSource.pageData.form, self.form.primaryKeys);
        ...
        for (var i in self.tabs) {
            ...
            if (tab.type == self.tabConst.grid) {
                ...
                tabData = comm.formChanges(self.pageData[name], data.dataSource.pageData[name], tab.primaryKeys);
            }
    ...
    this.fnIsPageValid = function () {
        var formValid = comm.formValidate();
        if (!formValid)
            return '验证不通过，数据未保存！';
        ...
        for (var i in self.tabs) {
            ...
            if (tab.type == self.tabConst.grid) {
                ...
                //formValid = comm.formValidate(document.getElementById("xxx"));
            }
        }
    ...
    this.auditClick = function () {
        ...
        if (changes._changed) {
            comm.message('warning', '数据有修改，请保存后再审核！');
            return;
        }
 
        comm.auditDialogForEditVM(self.pageData.form, function (d) {
            comm.ajax({
                type: 'POST',
                url: self.urls.audit + self.pageData.scrollKeys.current(),
                data: JSON.stringify(d),
                success: function () {
                    comm.message('success', d.status == "passed" ? self.resx.auditPassed : self.resx.auditReject);
                    ko.mapping.fromJS(self.pageData.form, {}, data.dataSource.pageData.form);
                },
                error: function (e) {
                    comm.message('error', e.responseText);
    ...
    this.printClick = function () {
        comm.openTab('打印报表', '/report?area=mms&rpt=' + self.urls.report + '&BillNo=' + self.form.data.BillNo(), 'icon-printer_color');
    };
    ...
    this.init = function () {
        ...
        for (var i in self.tabs) {
            ...
            if (tab.type == self.tabConst.grid) {
                ...
                var edit = this[self.tabConst.edit + i] = new comm.editGridViewModel(grid);
