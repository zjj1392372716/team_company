...
(function ($, ko) {
    ...
    ko.creatEasyuiValueBindings({ type: 'datebox', event: 'onSelect', formatter: fmt.formatDate });
    ko.creatEasyuiValueBindings({ type: 'datetimebox', event: 'onChange', formatter: fmt.formatTime });
        ...
        var gridBinding = {
            update: function (element, valueAccessor, allBindingsAccessor, viewModel) {
                ...
                var handler = function () {
                    if (grid.data(gridType)) {
                        ...
                        var defaults = {
                            ...
                            pagination: false,      //翻页
                            pageSize: comm.getSetting("gridrows",20),
                ...
                grid[gridType] ? handler() : using([gridType], function () {
                    handler();
                    if (gridType=='treegrid') comm.loadCss('/Content/css/icon/icon.css', document, true);//解决图标被tree.css覆盖的问题
                });
            }
        };
    ...
    ko.bindingHandlers.inputwidth = {
        update: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            ...
            var resizeWidth = function () {
                ...
                $.each([
                    { selector: 'input.tf-txt', handler: function (jq) { jq.width(width - 8); } },            //pading3*2 + border1*2 = 8
